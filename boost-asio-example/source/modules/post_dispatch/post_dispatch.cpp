  ...
  boost::asio::post(
      ...
      [&ioc]
      {
        ...
        boost::asio::dispatch(ioc,
                              []
                              {
                                std::cout << "2. inner dispatch runs inline, "
                                             "before outer handler returns\n";
                              });
        ...
        boost::asio::post(
            ...
            []
            {
              std::cout << "4. inner post runs after outer handler returned\n";
            });

        // defer 语义同 post（排队），但向执行器提示这是
        // “当前工作的延续”，允许其做调度优化。
        boost::asio::defer(ioc,
                           [] { std::cout << "5. inner defer runs last\n"; });
